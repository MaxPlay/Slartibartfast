...
namespace Slartibartfast.Generators
{
  ...
  138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180};
        ...
        public double Noise(double xin, double yin)
        {
            ...
            double y0 = yin - Y0;
            // For the 2D case, the simplex shape is an equilateral triangle. Determine which simplex
            // we are in.
            int i1, j1; // Offsets for second (middle) corner of simplex in (i,j) coords
            if (x0 > y0) { i1 = 1; j1 = 0; } // lower triangle, XY order: (0,0)->(1,0)->(1,1)
            else { i1 = 0; j1 = 1; }      // upper triangle, YX order: (0,0)->(0,1)->(1,1)
                                          // A step of (1,0) in (i,j) means a step of (1-c,-c) in
                                          // (x,y), and a step of (0,1) in (i,j) means a step of
                                          // (-c,1-c) in (x,y), where c = (3-sqrt(3))/6
        ...
        public double Noise(double xin, double yin, double zin)
        {
            ...
            // A step of (1,0,0) in (i,j,k) means a step of (1-c,-c,-c) in (x,y,z), a step of (0,1,0)
            // in (i,j,k) means a step of (-c,1-c,-c) in (x,y,z), and a step of (0,0,1) in (i,j,k)
        ...
        public double Noise(double x, double y, double z, double w)
        {
            ...
            double z0 = z - Z0;
            double w0 = w - W0;
            // For the 4D case, the simplex is a 4D shape I won't even try to describe. To find out
            // which of the 24 possible simplices we're in, we need to determine the magnitude
            // ordering of x0, y0, z0 and w0. Six pair-wise comparisons are performed between each
            // possible pair of the four coordinates, and the results are used to rank the numbers.
            int rankx = 0;
            int ranky = 0;
            int rankz = 0;
            int rankw = 0;
            ...
            int i3, j3, k3, l3; // The integer offsets for the fourth simplex corner
                                ...
                                // non-zero entries make any sense. We use a thresholding to set the
                                // coordinates in turn from the largest magnitude. Rank 3 denotes the
                                // largest coordinate.
            i1 = rankx >= 3 ? 1 : 0;
            j1 = ranky >= 3 ? 1 : 0;
            k1 = rankz >= 3 ? 1 : 0;
